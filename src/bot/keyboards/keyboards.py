...
def get_main_keyboard(user_id: int, admin_ids: List[int] = None, webapp_url: str = None) -> InlineKeyboardMarkup:
    ...
    if is_admin(user_id, admin_ids):
        buttons.extend([
            [InlineKeyboardButton(text="━━━ Админ-панель ━━━", callback_data="noop")],
...
def get_carwashes_keyboard(carwashes: List[dict], page: int = 1, total_pages: int = 1) -> InlineKeyboardMarkup:
    buttons = []
    for cw in carwashes:
        dist = f" ({cw['distance']:.1f} км)" if cw.get('distance') else ""
        buttons.append([InlineKeyboardButton(text=f"🏢 {cw['name']}{dist}", callback_data=f"carwash_{cw['id']}")])
    
    nav = []
    if page > 1: nav.append(InlineKeyboardButton(text="◀️", callback_data=f"cw_page_{page-1}"))
    if page < total_pages: nav.append(InlineKeyboardButton(text="▶️", callback_data=f"cw_page_{page+1}"))
...
def get_date_keyboard(carwash_id: str, days_ahead: int = 7) -> InlineKeyboardMarkup:
    ...
    for i in range(days_ahead):
        ...
        text = "Сегодня" if i == 0 else "Завтра" if i == 1 else f"{weekdays[d.weekday()]}, {d.day}"
        row.append(InlineKeyboardButton(text=text, callback_data=f"date_{carwash_id}_{d.isoformat()}"))
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row: buttons.append(row)
    
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data=f"carwash_{carwash_id}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def get_time_slots_keyboard(carwash_id: str, slot_date: str, slots: List[dict]) -> InlineKeyboardMarkup:
    buttons, row = [], []
    for slot in slots:
        row.append(InlineKeyboardButton(text=f"⏰ {slot['start_time']}", callback_data=f"slot_{slot['id']}"))
        if len(row) == 3:
            buttons.append(row)
            row = []
    if row: buttons.append(row)
    if not slots: buttons.append([InlineKeyboardButton(text="😔 Нет слотов", callback_data="no_slots")])
    buttons.append([InlineKeyboardButton(text="🔙 Другая дата", callback_data=f"select_date_{carwash_id}")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def get_wash_types_keyboard(wash_types: List[dict], slot_id: str) -> InlineKeyboardMarkup:
    buttons = []
    ...
    buttons.append([InlineKeyboardButton(text="🔙 Назад", callback_data="back_to_slots")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def get_my_bookings_keyboard(bookings: List[dict], show_active: bool = True) -> InlineKeyboardMarkup:
    buttons = [[
        InlineKeyboardButton(text="✅ Активные" if show_active else "✅ Активные", callback_data="noop" if show_active else "my_bookings"),
        InlineKeyboardButton(text="📜 История", callback_data="bookings_history" if show_active else "noop"),
    ]]
    
    icons = {'pending_payment': '⏳', 'confirmed': '✅', 'in_progress': '🔄', 'completed': '✔️', 'cancelled': '❌'}
    for b in bookings:
    ...
    if not bookings: buttons.append([InlineKeyboardButton(text="У вас нет бронирований", callback_data="noop")])
    buttons.append([InlineKeyboardButton(text="🔙 В меню", callback_data="back_to_menu")])
    return InlineKeyboardMarkup(inline_keyboard=buttons)


def get_booking_detail_keyboard(booking: dict, can_cancel: bool = True) -> InlineKeyboardMarkup:
    buttons, status = [], booking.get('status', '')
    
    if status == 'pending_payment':
        buttons.append([InlineKeyboardButton(text="💳 Оплатить", callback_data=f"pay_booking_{booking['id']}")])
    if status == 'confirmed':
        buttons.append([InlineKeyboardButton(text="📱 Показать QR", callback_data=f"show_qr_{booking['id']}")])
    ...
    buttons.extend([
        [InlineKeyboardButton(text="📍 Как добраться", callback_data=f"navigate_{booking['id']}")],
...
def get_booking_confirm_keyboard(booking_data: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=f"💳 Оплатить {prepayment:.0f}₽ (50%)", callback_data=f"pay_{booking_data.get('temp_id', 'new')}")],
...
def get_confirm_cancel_keyboard(booking_id: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[[
        ...
        InlineKeyboardButton(text="❌ Нет", callback_data=f"booking_{booking_id}"),
    ]])
