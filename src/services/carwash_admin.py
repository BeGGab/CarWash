...
class CarWashAdminService:
    def __init__(
        ...
        repo: CarWashAdminRepository = CarWashAdminRepository(),
    ...
    async def add_admin(
        ...
        user = await self.user_repo.find_by_phone(self.session, phone_number)
        if not user:
            raise ValueError("Пользователь с таким номером телефона не найден.")

        existing = await self.repo.get_by_user_and_carwash(
            self.session, user.telegram_id, car_wash_id
        )
        if existing:
            raise ValueError("Этот пользователь уже является администратором этой мойки.")

        admin_data = SCarWashAdminCreate(user_id=user.telegram_id, car_wash_id=car_wash_id)
        new_admin = await self.repo.add(self.session, admin_data)
        return new_admin

    async def get_user_roles(self, user_id: int) -> list[CarWashAdmin]:
